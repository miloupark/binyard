...
- HTTP: 데이터를 암호화하지 않고 전송하기 때문에, 네트워크 중간에서 공격자가 그대로 볼 수 있다.
- HTTPS: SSL/TLS 암호화를 적용하여, 공격자가 가로채더라도 실제 비밀번호 내용을 알 수 없다.
...
- 신뢰성: 브라우저 주소창에 자물쇠 아이콘이 표시되어 사용자에게 안전한 사이트라는 신뢰를 준다.
