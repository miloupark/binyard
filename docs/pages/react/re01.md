...
- 화면 업데이트 구현이 쉽고, 업데이트가 빠르게 처리된다.
- 상태(State)와 이벤트 기반의 동적 웹 앱 제작에 적합하다.

### React Application

React로 만든 웹 서비스들은 보통 React App, React Application 불린다.  
단순한 웹 페이지라기보다는, `브라우저에서 실행되는 하나의 애플리케이션`처럼 동작하기 때문이다.

전통적인 웹 페이지는 사용자가 다른 페이지로 이동할 때마다 서버에서 새로운 HTML을 받아왔다.  
하지만 React App은 `SPA(Single Page Application)` 방식으로,  
처음 한 번만 HTML과 JS 번들을 불러오고 이후에는 `JavaScript가 화면을 동적으로 갱신`한다.
...
- ### 선언적 프로그래밍 (Declarative Programming):
  ...
  "어떻게(How)"가 아닌 `"무엇(What)"`을 중심으로 코드를 작성하는 방식이다.  
   특정 변수의 값(state)을 바꾸는 것 만으로도 화면을 업데이트 시킬 수 있다.
  ...
  ::: info 선언형 vs 명령형 — React vs DOM 조작
...
- ### 가상 DOM 사용 (Virtual DOM):
  ...
  상태 변화가 발생하면, 먼저 `가상의 DOM 트리`를 만들고 이전 상태와 비교한 후
  ...
  - 성능 최적화: 직접 DOM 조작은 무겁지만, 가상 DOM 비교 후 변경점만 반영하면 빠르다.
  - 개발 편의성: 개발자가 DOM을 직접 조작하는 번거로움 없이 선언형 UI 작성 가능하다.
  - 일관성: 상태 기반 렌더링이라 예측 가능한 UI 업데이트

<br>

## React 프로젝트 생성

### CRA, Create-React-App <badge type="danger" text="⛔️ 비권장"></badge>
...
### Vite <badge type="tip" text="공식문서 권장"></badge>
...
::: info React App 웹 서버 · localhost

Vite를 통해 생성한 리액트 앱에는 웹 서버가 내장되어있다. React App 서버를 가동시켜라라는 의미의 `npm run dev` 명령어를 통해 콘솔에 출력되는 메세지는 리액트 웹 서버의 가동이 시작되었다는 의미로, 함께 출력되는 주소는 현재 가동중인 리액트 웹 서버에 접속할 수 있는 주소이다.

웹 브라우저에서 localhost:5173으로 접속하면, 내 컴퓨터에서 실행되고 있는 리액트의 웹 서버에 접속 요청을 보낸다. :5173은 포트 번호를 의미한다. 고유한 포트 번호를 붙여준다.
:::
...
- index.html에서 main.js 자바스크립트 파일을 불러와서 실행한다.
- main.js에서는 App.jsx 파일에 저장되어있는 App 함수가 리턴하는 요소를 화면에 렌더한다.  
  이 과정으로 리액트가 동작한다.
...
- HTML의 #root 요소를 찾아 `<App />` 컴포넌트를 렌더링한다.
