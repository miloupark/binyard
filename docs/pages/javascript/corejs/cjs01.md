...
- ① 변수 영역의 빈 공간 `@1002`를 확보하고, 변수 이름을 `obj1`로 지정한다.
- ② obj1에 할당할 값은 여러 개의 프로퍼티를 가진 객체이므로, 데이터 영역의 주소 `@5001`에 "객체를 저장"한다.
- ③ 객체 내부의 프로퍼티를 담기 위해, 별도의 변수 영역 `@7103~` 을 생성하고, 해당 주소를 `@5001`에 기록한다.
- ④ `@7103`, `@7104`에 각각 프로퍼티 이름 a, b를 저장한다.
- ⑤ 프로퍼티 a의 값인 1은 데이터 영역에 존재하지 않으므로 새 주소 `@5003`에 저장하고, 이 주소를 `@7103`에 연결한다.
  프로퍼티 b의 값 'bbb'도 마찬가지로 `@5004`에 저장하고, 이를 `@7104`에 연결한다.
